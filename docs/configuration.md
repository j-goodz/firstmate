...
Unlike `COMPACT_ADVISER_DISABLE`, the name is deliberately absent from the cleared-environment floor list used by `config/launch-env-allowlist`, so the launch command's own export is the single delivery for a worker in either allowlist posture, and a secondmate simply starts without the variable set.
