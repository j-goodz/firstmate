...
Unlike `COMPACT_ADVISER_DISABLE`, the name is deliberately absent from the cleared-environment floor list used by `config/launch-env-allowlist`, so under an enabled allowlist the launch command's own export is the whole delivery for a worker, and a secondmate simply starts without the variable set.
